...
//converts binary to string based on length
string ASMParser::cvtBinaryToString(int i, int len)
{	
	//know that operand doesnt exist
	string s;
	if(i == -1)
	{
		for(int i =0; i<len; i++)
  ...
  //operand does exist
	else 
	{
		...
		{
			return bitset<16>(i).to_string();
		}
		else 
		{
			return bitset<26>(i).to_string();
		}
  ...
  // of that instruction.
{
  ...
  ss << type;
  string typeOf = ss.str();


 if(type == RTYPE)
 {
  //create RTYPE instruction
  s.append(opcodes.getOpcodeField(opCode));
  s.append(cvtBinaryToString(rs, 5));
  s.append(cvtBinaryToString(rt, 5));
  s.append(cvtBinaryToString(rd, 5));
  s.append(cvtBinaryToString(imm, 5));
  s.append(opcodes.getFunctField(opCode));
 ...
 {
  //create ITYPE instruction
  s.append(opcodes.getOpcodeField(opCode));
  s.append(cvtBinaryToString(rs, 5));
  s.append(cvtBinaryToString(rt, 5));
  s.append(cvtBinaryToString(imm, 16));
 }
 ...
 else if(type == JTYPE)
 {
  //create JTYPE 
  s.append(opcodes.getOpcodeField(opCode));
  string immd = cvtBinaryToString(imm, 26);
  immd = "00" + immd;
  immd = immd.substr(0,26);
  s.append(immd);
